...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `description` varchar(100) DEFAULT `no description` PRIMARY KEY (`id`) -- Primary keys are UNIQUE and NOT NULL
...
-- Copy the structure, but not the values
...
CREATE TABLE `cat2`(
  `name` VARCHAR(100) DEFAULT "kitten",
  -- default value is given if no name is provided
  `age` INT DEFAULT 99
) CREATE TABLE `cat3`(
...
CREATE TABLE `orders`(
  ...
  `amount` DECIMAL(8, 2),
  `customer_id` INT,
  -- values inserted here are validated -> must match with any id in customer table
  FOREIGN KEY(`customer_id`) REFERENCES `customers`(`id`) -- FK ensures that records with no reference to the parent table are NOT inserted
